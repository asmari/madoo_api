	...
	schema: {
		...
		body: {
			...
			properties: {
				...
				email: { type: 'string' },
				mobile_phone: { type: 'string', maxLength: 15, minLength: 8 },
	...
	schema: {
		...
		body: {
			properties: {
				...
				mobile_phone: {
					type: 'string', maxLength: 15, minLength: 8,
				},
			},
			required: ['full_name'],
		},
...
exports.removeTokenSchema = {
	schema: {
		...
		description: 'Rest API for remove token user',
		body: {
			required: ['user_id'],
			properties: {
				user_id: {
